...
mutate(
  ...
  scale_x_continuous(
    ...
    theme_minimal()
# we can not use final_gap_data because otherwise we do not have the required periods since this only has 2014 and 2019, that is why I used obesity_trend_clean because now we have more than enough periods.
# This filters only the selected countries from the dataset
# I changed the name of the shortened country name to the full country name, so FR becomes France in the legend of the graph.
# this plots the obesity over time, x is the years on the x-axis, y is the obesity rate on the y-axis, color equals geo gives every country a different color, and group=geo connects all the lines.
# the codes show how the lines are shown for example the width etc, it also creates the titles and the info of everything that is shown in the visualisation.
...
spatial_map <- europe_map %>%
  left_join(gap_map_data, by = "iso_a2")
